...
const imageRefsMap = new Map<
  number,
  Konva.Image | Konva.Text | Konva.Group | null
>();

export const getImageRef = (index: number) => imageRefsMap.get(index) || null;
export const setImageRef = (
  index: number,
  ref: Konva.Image | Konva.Text | Konva.Group | null,
) => {
    ...
    (set, get) => ({
      ...
      loadBrief: async (briefId) => {
        ...
        try {
          ...
          if (
            brief.canvasState?.images &&
            brief.canvasState.images.length > 0
          ) {
            // Load images from S3 URLs
            const loadedImages = await Promise.all(
              brief.canvasState.images.map(
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                async (imgData: any, index: number) => {
      ...
      addImage: (image) => {
        set((state) => {
          ...
          const isReaction =
            image.sourceType === "sticker" ||
            image.sourceType === "postit" ||
